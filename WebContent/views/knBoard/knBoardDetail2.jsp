...
<link rel="stylesheet" type="text/css" href="<%= request.getContextPath() %>/css/index.css"/>
	<style>
		...
		.writeArea > table{margin-left: 30px; margin-top: 3px; min-height: 550px;}
		.writeArea >table, th, td{word-spacing: 3px; padding: 4px;}
		.writeArea >table > tfoot > th, td{padding-top: 5px;}
		...
		#deleteBtn{font-size: 15px; text-align: center; border: 1px solid #575756; border-radius: 5px; width: 80px; height: 35px; cursor: pointer;}
						...
						<% } else { %>
							<% for(int i = 0; i < list.size(); i++){ %>
								...
								<tr>
									...
									<td width="200px"><%= list.get(i).getKnrDate() %></td>
								</tr>
								</tbody>				
							<% } %>						
						<% } %>						
	    		...
	    		$('#addReply').click(function(){	
					...
					$.ajax({
						...
						success: function(data){
							$replyTable = $('#replySelectTable tbody');
							$replyTable.html("");
							for(var i in data){
								var $tr = $("<tr>");
								var $writerTd = $("<td>").text(data[i].getUsId);
								var $contentTd = $("<td>").text(data[i].KnrCon);
								var $dateTd = $("<td>").text(data[i].KnrDate);
