			...
			<form method="get">
				...
				<div class="tableTitle">
					...
					<div class="pagingArea" align="center">
						...
						%>
						<button
							onclick="location.href='<%=request.getContextPath()%>/list.ms?currentPage=1'">&lt;&lt;</button>
	
	
						<!-- 이전 페이지 -->
						<button
							onclick="location.href='<%=request.getContextPath()%>/list.ms?currentPage=<%=currentPage - 1%>'"
							id="beforeBtn">&lt;</button>
						<!-- 이전 페이지 갈 작동 함수 필요 -->
						<script>
						if(<%=currentPage%> <= 1){
						...
						%>
						<button id="numBtn"
							onclick="location.href='<%=request.getContextPath()%>/list.ms?currentPage=<%=p%>'"><%=p%></button>
						<%
							}
						%>
						<%
						...
						<!-- 맨 끝으로 -->
						<button
							onclick="location.href='<%=request.getContextPath()%>/list.ms?currentPage=<%=maxPage%>'">&gt;&gt;</button>
						<%
							}
						%>
					</div>
