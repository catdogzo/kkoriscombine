	...
	<div class="container">
    	<section class="Nmain">
			<article class="NquickMenu">
				...
				<form action="<%=request.getContextPath() %>/noticeUpdate.bo" class=noticeInsertC>
					<br>
					<div class=title>
						<input type=hidden name=noNum value="<%=noNum %>">
						<label>제목 </label><input type=text name=title value="<%=title %>" class=title>
					</div>
					...
					<div class=content>
						...
						<textarea wrap="hard" name=content class=content><%=content %></textarea>
					</div>
					...
					<div class=insertNotice-submit>
						...
						<input type=button class=cancelBtn value=취소 onclick="location.href='<%=request.getContextPath()%>/noticeDetailView.bo?noNum=<%=noNum%>'">
					</div>
				</form>
			</article>
		</section>
